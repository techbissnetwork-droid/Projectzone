...
const related = services.filter((s) => ["ssl-security", "business-email", "website-development"].includes(s.slug));
    ...
    <>
      <Section className="pb-8 pt-14 sm:pt-20" aria-label={service.title}>
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-2 lg:gap-10">
          <Reveal>
            <Eyebrow>
              <DetailIcon className="h-3.5 w-3.5" aria-hidden="true" /> Infrastructure
            </Eyebrow>
      ...
      <Section aria-label="What's included" className="border-y border-white/5 bg-white/[0.015]">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-2 lg:gap-16">
          <Reveal>
            <SectionHeading eyebrow="What's included" title="A stable foundation, configured correctly" />
          </Reveal>
          <Reveal delay={0.1}>
      ...
      <Section aria-label="How it works" className="border-y border-white/5 bg-white/[0.015]">
        <Reveal>
          <SectionHeading eyebrow="How it works" title="From domain search to monitored uptime" />
        </Reveal>
        <div className="mt-10">
          <MiniProcess
            steps={[
              { title: "Domain search & registration", description: "We help you secure the right domain and register it correctly under your business." },
              { title: "DNS & hosting setup", description: "Hosting environment provisioned on high-availability infrastructure with CDN configured." },
              { title: "Backup & monitoring", description: "Automated backups and uptime monitoring configured from day one." },
              { title: "Handover", description: "Full documentation so your team knows exactly what's running and where." },
            ]}
          />
        </div>
      </Section>
      ...
      <CtaSection
        title="Get your domain and hosting set up right the first time."
        description="No half-configured DNS, no surprise downtime — just a stable foundation."
      />
    </>
  );
}
