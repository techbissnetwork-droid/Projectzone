...
**Admin → Search engines** checks all of this against your live site from the
browser — home page, `robots.txt`, your sitemap, and whether `/index.php`
redirects — and tells you the exact address to submit to Google. It also writes
`robots.txt` for you.

Where the `.htaccess` rewrite is in force, `/robots.txt` and `/sitemap.xml` are
generated by `robots.php` and `sitemap.php`. Where it is not, a plain
`robots.txt` ships with the site so that address never 404s (crawlers treat an
unreadable robots.txt as a reason to back off), and the sitemap is served at
`/sitemap/` — the address the admin page tells you to submit. On nginx, map
