/* ==========================================================================
   TECHBISS — Portfolio Pulse — interaction system
   Nav/reveal/counter/accordion system shared with the other concepts, plus
   two additions specific to this design: a live "Slide X / Y" counter for
   the work filmstrip (derived from scrollLeft vs. card width) and a
   floating scroll-to-top button that appears past one viewport height.
...
(function(){
  ...
  /* ---------------- Scroll reveals (also activates any device-frame charts) ---------------- */
  ...
  /* ---------------- Filmstrip: drag-scroll + prev/next + live "Slide X / Y" ---------------- */
  document.querySelectorAll('.filmstrip').forEach(function(strip){
    ...
    var cards = strip.querySelectorAll('.work-card');
    var step = function(){ return (cards[0] && cards[0].offsetWidth + 21) || 320; };
    ...
    if (wrap) {
      ...
      var counterCurrent = wrap.querySelector('[data-slide-current]');
      var counterTotal = wrap.querySelector('[data-slide-total]');
      if (counterTotal && cards.length) counterTotal.textContent = cards.length;

      var rafPending = false;
      function updateSlideCounter(){
        rafPending = false;
        if (!counterCurrent || !cards.length) return;
        var width = step();
        var maxScroll = strip.scrollWidth - strip.clientWidth;
        var idx;
        if (maxScroll <= 0) idx = 0;
        else idx = Math.round(strip.scrollLeft / width);
      ...
      function requestSlideUpdate(){
        ...
        rafPending = true;
        requestAnimationFrame(updateSlideCounter);
      }
      updateSlideCounter();
      strip.addEventListener('scroll', requestSlideUpdate, { passive: true });
      strip.addEventListener('pointermove', function(){ if (isDown) requestSlideUpdate(); });
  ...
  if (scrollTopBtn) {
    function toggleScrollTop(){
      scrollTopBtn.classList.toggle('is-visible', window.scrollY > window.innerHeight);
    }
    toggleScrollTop();
    ...
    scrollTopBtn.addEventListener('click', function(){
      ...
      else if (lenis) { lenis.scrollTo(0, { duration: 1.0 }); }
