/* ==========================================================================
   TECHBISS — Command Palette — motion system
   Standard nav/reveal/counter/filmstrip system shared across concepts, plus
   the hero's typewriter-style cycling placeholder for the fake command bar.
...
(function(){
  ...
  /* ---------------- Command bar: typewriter-cycling placeholder ----------------
     Writes directly to the real `placeholder` attribute on a timed loop — no
     overlay element, no library. Reduced-motion just shows one static example. */
  var commandInput = document.querySelector('[data-command-input]');
  if (commandInput) {
    var queries = [
      'a website for my restaurant',
      'an inventory system',
      'online payments for my store',
      'a booking app',
      'a client portal'
    ];
    if (reduced) {
      commandInput.placeholder = queries[0];
    } else {
      (function(){
        var qIndex = 0;
        var charIndex = queries[0].length;
        var phase = 'hold';
        var TYPE_MS = 38, DELETE_MS = 24, HOLD_MS = 1500, GAP_MS = 350;
        var timer = null;

        function step(){
          try {
            var word = queries[qIndex];
            if (phase === 'hold') {
              phase = 'deleting';
            ...
            if (phase === 'deleting') {
              charIndex--;
              commandInput.placeholder = word.slice(0, Math.max(charIndex, 0));
              if (charIndex <= 0) {
            ...
            if (phase === 'typing') {
              charIndex++;
              commandInput.placeholder = queries[qIndex].slice(0, charIndex);
              if (charIndex >= queries[qIndex].length) {
                phase = 'hold';
            ...
            /* Fail silent — whatever placeholder text is already showing just stays put. */
