/* ==========================================================================
   TECHBISS — Big Type — motion system
   Drives the word-by-word headline cascade (via the same generic
   [data-reveal]/[data-reveal-delay] system used for every other reveal on
   the page — each hero word is just a small reveal target with a hand-set
   delay), plus the standard nav/reveal/counter/accordion/filmstrip system.
...
(function(){
  ...
  /* ---------------- Scroll reveals ----------------
     Every [data-reveal] element fades/slides in once it enters view. Each
     hero word is wrapped as its own [data-reveal] span with a hand-authored
     data-reveal-delay (see index.html), so this single generic system is
     what produces the headline's word-by-word cascade — no bespoke code
     needed for it. Reduced motion (or no IntersectionObserver support)
     reveals everything immediately, with zero stagger. */
  ...
  /* ---------------- Accordion (also powers the big-type services list) ---------------- */
    ...
    trigger.addEventListener('click', function(){
      ...
      if (group) group.querySelectorAll('.accordion-item.is-open').forEach(function(other){
        if (other !== item) {
          ...
          other.querySelector('.accordion-panel').style.maxHeight = '';
  ...
  /* ---------------- Subtle scroll parallax on the hero's background marquee ----------------
     Purely decorative, purely additive on top of the marquee's own CSS drift —
     if GSAP/ScrollTrigger fail to load or reduced motion is set, the marquee
     still works fine via CSS alone, so this is wrapped defensively. */
  if (hasGSAP && window.ScrollTrigger && !reduced) {
    var heroEl = document.querySelector('.hero');
    var topRow = document.querySelector('.hero-marquee.is-top');
    var bottomRow = document.querySelector('.hero-marquee.is-bottom');
    if (heroEl && (topRow || bottomRow)) {
      if (topRow) gsap.to(topRow, { yPercent: 30, ease: 'none', scrollTrigger: { trigger: heroEl, start: 'top top', end: 'bottom top', scrub: true } });
      if (bottomRow) gsap.to(bottomRow, { yPercent: -30, ease: 'none', scrollTrigger: { trigger: heroEl, start: 'top top', end: 'bottom top', scrub: true } });
    }
  }
